...
class Leaf {
  constructor(loc, branchAngle) {    
    ...
    this.color = { start: p5.color("PaleGreen"), end: p5.color("PaleGoldenrod") };
    this.outlineColor = { start: p5.color("Chartreuse"), end: p5.color("DarkGreen") };
  ...
  setToInitial(location = window.p5instance.createVector(0, 0), branchAngle) {  
    const p5 = window.p5instance
    this.subLeaves = [];
    this.subLeaves.push(new SubLeaf("son1", p5.createVector(38, p5.random(-7, 7))));
    this.subLeaves.push(new SubLeaf("son2", p5.createVector(-38, p5.random(-7, 7))));
    this.subLeaves.push(new SubLeaf("son3", p5.createVector(p5.random(-7, 7), -50)));

    this.currentStep = maxSteps;
    const side = p5.random([60, -60]);
    this.stemAngle = p5.radians(p5.degrees(branchAngle) + side + p5.random(-45, +45));
